...
## Quotes (VR-068–VR-073)

| Code | Scope | Pass condition |
|------|-------|----------------|
| VR-068 | Quote | Unique `quoteId` / `quoteNumber`; ≥8 quotes; `status` in `Draft` \| `Sent` \| `Accepted` \| `Declined` \| `Expired`; `currency` is `GBP`; `quoteNumber` matches `QUO-YYYY-nnnn`; 2–5 nested lines each |
...
| VR-073 | Quote | Authored money: line totals, subtotal, tax (from snapshot rates), total, and `expiryDate` ≥ `issueDate` |
