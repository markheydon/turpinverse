...
- [ ] T001 Create empty canon array file `professional-extras.json` (`[]`) in `src/Turpinverse.Data/canon/`
...
- [ ] T003 Confirm feature branch `006-professional-profile-extras` is active and `specs/006-professional-profile-extras/` design artifacts are present
...
- [ ] T004 [P] Add failing CanonValidator professional extras fixture tests (VR-036–VR-043) in `tests/Turpinverse.Core.UnitTests/Validation/ProfessionalExtrasValidatorTests.cs` with `[Trait("Category", "CanonValidation")]`
- [ ] T005 [P] Add failing CanonSchemaValidator tests for IntroCopy, ContactExtras, ProfessionalSocial, and ProfessionalExtras in `tests/Turpinverse.Core.UnitTests/Validation/ProfessionalExtrasSchemaValidatorTests.cs`
- [ ] T006 [P] Add failing JsonCanonRepository load test for `professional-extras.json` in `tests/Turpinverse.Core.UnitTests/Data/JsonCanonRepositoryTests.cs`
- [ ] T007 [P] Add failing ProfessionalExtrasPresenter lookup and visibility-flag tests in `tests/Turpinverse.Core.UnitTests/Profile/ProfessionalExtrasPresenterTests.cs`

### Implementation for Foundational

- [ ] T008 [P] Create `IntroCopy` model in `src/Turpinverse.Core/Models/IntroCopy.cs`
- [ ] T009 [P] Create `ContactExtras` model in `src/Turpinverse.Core/Models/ContactExtras.cs`
- [ ] T010 [P] Create `ProfessionalSocial` model in `src/Turpinverse.Core/Models/ProfessionalSocial.cs`
- [ ] T011 Create `ProfessionalExtras` model in `src/Turpinverse.Core/Models/ProfessionalExtras.cs`
- [ ] T012 Extend `Canon` with `ProfessionalExtras` collection in `src/Turpinverse.Core/Models/Canon.cs`
- [ ] T013 Update `JsonCanonRepository` to load `professional-extras.json` in `src/Turpinverse.Data/Repositories/JsonCanonRepository.cs`
- [ ] T014 Merge additive types from `specs/006-professional-profile-extras/contracts/professional-extras-schema.json` into `specs/001-turpinverse-universe/contracts/canon-schema.json` (`professionalExtras` required array and `$defs`)
- [ ] T015 Extend `CanonSchemaValidator` for professional extras entity types in `src/Turpinverse.Core/Validation/CanonSchemaValidator.cs`
- [ ] T016 Implement VR-036–VR-043 validation methods and add `professionalExtras` to result counts in `src/Turpinverse.Core/Validation/CanonValidator.cs`
- [ ] T017 Create `ProfessionalExtrasPresenter` with lookup, author-order lists, and title/biography/header-email visibility flags in `src/Turpinverse.Core/Profile/ProfessionalExtrasPresenter.cs`
- [ ] T018 Register `ProfessionalExtrasPresenter` in `src/Turpinverse.Core/DependencyInjection/ServiceCollectionExtensions.cs`
...
- [ ] T019 [P] [US1] Add failing HugoContentGenerator intro extras emission tests in `tests/Turpinverse.Core.UnitTests/Hugo/ProfessionalExtrasHugoTests.cs` with `[Trait("Category", "ProfessionalExtras")]`
- [ ] T020 [P] [US1] Add failing Blazor intro header and title-suppression tests in `tests/Turpinverse.Web.UnitTests/Components/ContactDetailTests.cs` with `[Trait("Category", "ProfessionalExtras")]`

### Implementation for User Story 1

- [ ] T021 [US1] Author intro block for `dick-turpin` in `src/Turpinverse.Data/canon/professional-extras.json` (seed `shortIntro`, `headline`, `subtitle` from display name, title, biography; optional photo and single CTA allowed)
- [ ] T022 [US1] Extend `HugoContentGenerator` to emit extras into `site/data/` keyed by persona id in `src/Turpinverse.Core/Hugo/HugoContentGenerator.cs`
- [ ] T023 [US1] Add intro header section and suppress competing `jobTitle` when intro present in `site/layouts/personas/single.html`
- [ ] T024 [US1] Extend `ContactDetail.razor` with intro header and title suppression using `ProfessionalExtrasPresenter` in `src/Turpinverse.Web/Components/Pages/ContactDetail.razor`
- [ ] T025 [P] [US1] Create `IntroSection.razor` in `src/Turpinverse.Web/Components/Profile/IntroSection.razor`
- [ ] T026 [P] [US1] Add intro header styles in `src/Turpinverse.Web/Styles/app.css` (or `wwwroot/app.css` if that is the build source)
...
- [ ] T027 [P] [US2] Add failing Hugo about/skills sandwich-order tests in `tests/Turpinverse.Core.UnitTests/Hugo/ProfessionalExtrasHugoTests.cs`
- [ ] T028 [P] [US2] Add failing Blazor about/skills and biography-suppression tests in `tests/Turpinverse.Web.UnitTests/Components/ContactDetailTests.cs`

### Implementation for User Story 2

- [ ] T029 [US2] Add `about`, `skillsHeading`, and ≥5 unique `skills` to `dick-turpin` row in `src/Turpinverse.Data/canon/professional-extras.json` (names only, author order, no career/portfolio lists)
- [ ] T030 [US2] Add about and skills sections before career blocks in `site/layouts/personas/single.html` (omit empty; preserve author order; suppress biography when about present)
- [ ] T031 [P] [US2] Create `AboutSection.razor` in `src/Turpinverse.Web/Components/Profile/AboutSection.razor`
- [ ] T032 [P] [US2] Create `SkillsSection.razor` in `src/Turpinverse.Web/Components/Profile/SkillsSection.razor`
- [ ] T033 [US2] Integrate about and skills into `ContactDetail.razor` in sandwich order before career sections with biography suppression
...
- [ ] T034 [P] [US3] Add failing Hugo contact/socials placement-after-career tests in `tests/Turpinverse.Core.UnitTests/Hugo/ProfessionalExtrasHugoTests.cs`
- [ ] T035 [P] [US3] Add failing Blazor contact/socials and header-email-suppression tests in `tests/Turpinverse.Web.UnitTests/Components/ContactDetailTests.cs`

### Implementation for User Story 3

- [ ] T036 [US3] Complete `contact` and ≥3 unique `socials` on `dick-turpin` row in `src/Turpinverse.Data/canon/professional-extras.json` (email matches persona; optional phone/CTA; no forbidden keys)
- [ ] T037 [US3] Add contact and socials sections after career blocks in `site/layouts/personas/single.html` (omit empty; suppress header email when contact present)
- [ ] T038 [P] [US3] Create `ContactExtrasSection.razor` in `src/Turpinverse.Web/Components/Profile/ContactExtrasSection.razor`
- [ ] T039 [P] [US3] Create `SocialsSection.razor` in `src/Turpinverse.Web/Components/Profile/SocialsSection.razor`
- [ ] T040 [US3] Integrate contact and socials into `ContactDetail.razor` after career sections with header-email suppression
...
- [ ] T041 Bump `canon.json` version in `src/Turpinverse.Data/canon/canon.json` when professional extras dataset ships
- [ ] T042 Run `dotnet test tests/Turpinverse.Core.UnitTests --filter "Category=CanonValidation"` and resolve failures
- [ ] T043 [P] Run `dotnet test tests/Turpinverse.Web.UnitTests --filter "Category=ProfessionalExtras"` and resolve failures
- [ ] T044 Regenerate Hugo content via `src/Turpinverse.Tools.GenerateHugoContent/Program.cs` and confirm `site/data/` extras files for `dick-turpin`
- [ ] T045 [P] Build Hugo site (`site/` with `hugo --minify` or `scripts/Invoke-HugoSite.ps1`) and spot-check `/personas/dick-turpin/`
- [ ] T046 Verify `/api/canon/validate` returns 200 with `professionalExtras` count per `specs/006-professional-profile-extras/contracts/completeness.md`
- [ ] T047 Update dual-publication inventory for professional extras on person pages in `docs/product-surfaces.md`
- [ ] T048 Execute quickstart.md Scenarios A–E (manual Scenarios A–D; Scenario E negative fixtures in test code only)
